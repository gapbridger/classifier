...
class LogisticRegression(object):
    ...
    def __init__(self, input, input_dim, output_dim):
        ...
        self.class_probabilities = T.nnet.softmax(T.dot(input, self.W) + self.b)
        # predict the class label by taking the maximum
        self.predicted_class = T.argmax(self.class_probabilities, axis=1)
    ...
    def negative_log_likelihood(self, y):
        ...
        # [0,1,2,... n-1] T.log(self.class_probabilities) is a matrix of
    ...
    def errors(self, y):
        ...
        # check if y has same dimension of predicted_class
        if y.ndim != self.predicted_class.ndim:
            raise TypeError(
                'y should have the same shape as self.predicted_class',
                ('y', y.type, 'predicted_class', self.predicted_class.type)
            )
...
def predict():
    ...
    predict_model = theano.function(
        ...
        outputs=classifier.predicted_class)
